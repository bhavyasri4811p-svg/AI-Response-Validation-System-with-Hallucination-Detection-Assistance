...
def evaluate_csv(csv_path):
    ...
    scores = [r["verdict"]["overall_score"] for r in results]

    count = len(results)
    pass_percentage = round((pass_count / count) * 100, 2)

    summary = {

    "total": count,

    "average_score": round(overall_total / count, 2),
    ...
    "average_relevance": round(relevance_total / count, 2),

    "average_accuracy": round(accuracy_total / count, 2),

    "average_hallucination": round(hallucination_total / count, 2),

    "average_completeness": round(completeness_total / count, 2)
     }
